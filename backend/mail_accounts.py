...
# „Das Modell soll den Stil selbst waehlen." Gilt ueberall, wo ein Stil gewaehlt
# werden kann – in der Antwort-Vorschau des Add-ins UND in einer Regel
# (ausdrueckliche Vorgabe des Nutzers, 2026-08-19). `stil_fuer` gibt den Wert
# als `quelle="auto"` heraus, OHNE Text; den Katalog baut der Aufrufer
# (`mail_runner`), weil nur er weiss, wie der Auftrag aussieht.
STIL_AUTO = "auto"
...
def stil_fuer(user: str, stil_id: str = "", prompt: str = "") -> dict:
    ...
    Reihenfolge – die Bedeutung steckt in ihr:
      1. ``stil_id`` ausdruecklich gewaehlt (Pulldown / Regelfeld) → dieser.
         ...
         "das Modell waehlt" (dann ``quelle="auto"`` ohne Text – der Aufrufer
         legt dem Modell die Stile zur Auswahl vor).
    ...
    if wahl == STIL_AUTO:
        # Kein fester Stil – der Aufrufer erkennt das an `quelle` und legt dem
        # Modell alle Stile zur Auswahl vor.
        return dict(leer, quelle="auto",
                    hinweis="" if liste else
                    "Es ist kein Antwort-Stil hinterlegt – es wirkt kein Stil.")
    if wahl:
        for e in liste:
            if e["id"] == wahl:
