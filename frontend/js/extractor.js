...
window.extractorManager = new (class JarvisExtractorManager {
    ...
    constructor() {
        this._pending     = [];   // Liste ausstehender Dokumente
        this._reviewing   = null; // Aktuell geöffnetes Review-Dokument
    ...
    _render() {
        ...
        <div class="kb-container">
            ...
            <!-- Info-Modal (Anleitung) -->
            <div id="ext-info-modal" class="modal-overlay hidden">
                <div class="modal-card" style="max-width:520px;text-align:left;">
                    ...
                    <div class="ext-info-step">
                        ...
                        <div>
                            ...
                            <span class="kb-hint" style="margin:0;">Jarvis ruft die Seite ab und analysiert den Inhalt mit dem konfigurierten LLM (Large Language Model). Dabei werden automatisch eine Zusammenfassung, Kernfakten und Frage-Antwort-Paare generiert.</span>
                        </div>
                    </div>
                    <div class="ext-info-step">
                    ...
                    <div style="background:rgba(255,255,255,0.04);border-radius:8px;padding:10px 14px;margin-top:4px;">
                        <p class="kb-hint" style="margin:0;">
                            <strong>Hinweis:</strong> Nicht gespeicherte Extraktionen bleiben im Bereich <em>Ausstehend</em> und können jederzeit geprüft oder verworfen werden. Der Extraktor funktioniert nur mit öffentlich erreichbaren Webseiten (kein Login erforderlich).
                        </p>
                    </div>
            ...
            <div class="kb-section">
                <div class="kb-section-header">
                    ...
                    <button id="ext-info-btn" class="kb-btn-secondary" title="Anleitung anzeigen">❓ Anleitung</button>
                </div>
            ...
            <div class="kb-section">
                <div class="kb-section-header">
                    <h3>Ausstehend <span id="ext-pending-count" class="ext-badge">0</span></h3>
            ...
            <div id="ext-review-wrap" class="kb-section" style="display:none;">
                ...
                <div class="kb-form-field" style="margin-bottom:10px;">
                    ...
                    <button id="ext-add-fact-btn" class="kb-btn-secondary" style="margin-top:6px;font-size:0.78rem;">+ Fakt hinzufügen</button>
                </div>
                ...
                <div class="kb-form-field" style="margin-bottom:10px;">
                    ...
                    <button id="ext-add-qa-btn" class="kb-btn-secondary" style="margin-top:6px;font-size:0.78rem;">+ Paar hinzufügen</button>
                </div>

                <!-- Footer -->
                <div class="kb-form-footer" style="margin-top:12px;">
                    <button id="ext-reject-btn"  class="kb-btn-danger"  style="padding:0.45rem 1rem;font-size:0.82rem;">❌ Verwerfen</button>
        ...
        // Info-Modal: Klick außerhalb schließt
        document.getElementById('ext-info-modal').addEventListener('click', e => {
            if (e.target.id === 'ext-info-modal') this._hideInfo();
        });
    }

    // ─── Info-Modal ──────────────────────────────────────────────────────────

    _showInfo() { document.getElementById('ext-info-modal').classList.remove('hidden'); }
    _hideInfo() { document.getElementById('ext-info-modal').classList.add('hidden'); }
    ...
    _renderPending() {
        ...
        el.innerHTML = this._pending.map(doc => {
            ...
            <div class="cron-item" data-id="${doc.id}">
                ...
                <div class="cron-item-meta">${domain} · ${dt} · ${fct} Fakten · ${qa} Q&amp;A-Paare</div>
            </div>`;
        }).join('');
    ...
    _openReview(doc) {
        this._reviewing = doc;
        const wrap = document.getElementById('ext-review-wrap');
        wrap.style.display = '';
    ...
    _renderFacts(facts) {
        ...
        el.innerHTML = facts.map((f, i) => `
            <div class="ext-fact-row" data-idx="${i}">
                ...
                <button class="kb-btn-danger ext-fact-del" data-idx="${i}" title="Löschen" style="padding:0.3rem 0.6rem;">✕</button>
            </div>`).join('');
    ...
    _renderQa(pairs) {
        ...
        el.innerHTML = pairs.map((p, i) => `
            <div class="ext-qa-row" data-id="${p.id || i}">
                <div class="ext-qa-header">
                    ...
                    <button class="kb-btn-danger ext-qa-del" data-idx="${i}" title="Löschen" style="padding:2px 7px;font-size:.73rem;margin-left:auto;">✕</button>
                </div>
        ...
        el.querySelectorAll('.ext-qa-del').forEach(btn => {
            btn.onclick = () => {
                ...
                this._reviewing.qa_pairs = pairs;
    ...
    _getQaPairs() {
        const rows = document.querySelectorAll('.ext-qa-row');
        return [...rows].map(row => {
            const id      = row.dataset.id;
            const check   = row.querySelector('.ext-qa-check');
            const qInput  = row.querySelector('.ext-qa-q');
            const aInput  = row.querySelector('.ext-qa-a');
            return {
                id:       id,
                q:        qInput?.value.trim() || '',
                a:        aInput?.value.trim() || '',
                approved: check?.checked ?? true,
            };
        }).filter(p => p.q || p.a);
    }
    ...
    async _approve() {
        ...
        const approvedCount = updated.qa_pairs.filter(p => p.approved).length;
        if (approvedCount === 0 && updated.facts.length === 0) {
        ...
        try {
            // 1. Update (aktuellen Stand sichern)
            ...
            if (!r.ok || !d.ok) {
                ...
                this._notify(`✅ Gespeichert: ${d.fact_count} Fakten + ${d.qa_count} Q&A-Paare → ${d.file}`);
                this._closeReview();
                this._loadPending();
            }
        } catch (e) {
